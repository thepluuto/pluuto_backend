...
{% block title %}Scan Results - Pluuto Admin{% endblock %}
...
<div class="table-container">
    <div class="description-header"
        ...
        <div style="display: flex; gap: 10px; align-items: center;">
            <span><i class="fa-solid fa-qrcode"></i> Scan Results (Scratch Cards)</span>
        </div>
    </div>
        ...
        <form method="get" style="display: flex; gap: 10px; flex-wrap: wrap; align-items: end;">
            ...
            <div>
                ...
                <select name="status" class="form-control" style="min-width: 150px; padding: 8px; font-size: 13px;">
                    ...
                    <option value="scratched" {% if request.GET.status == 'scratched' %}selected{% endif %}>Revealed</option>
                    <option value="unscratched" {% if request.GET.status == 'unscratched' %}selected{% endif %}>Pending</option>
                </select>
            </div>
            <button type="submit" class="action-btn btn-primary" style="padding: 8px 16px;">Filter</button>
            <a href="{% url 'admin_scan_result_list' %}" class="action-btn btn-outline" style="padding: 8px 12px; text-decoration: none;">Reset</a>
        </form>
    </div>
    ...
    <table class="custom-table">
        ...
        <tbody>
            {% for card in scan_results %}
            <tr>
                <td style="color: var(--text-secondary); font-family: monospace;">#{{ card.id }}</td>
                <td>
                    <div style="font-weight: 500;">{{ card.user.full_name|default:card.user.phone_number }}</div>
                    <div style="font-size: 12px; color: var(--text-secondary);">{{ card.user.email }}</div>
                </td>
                <td>
                    <div style="font-weight: 600; color: var(--text-primary);">{{ card.event.title }}</div>
                </td>
                <td style="font-weight: 600; color: var(--success-color);">{{ card.prize_text }}</td>
                <td>
                ...
                <td style="color: var(--text-secondary);">{{ card.created_at|date:"M d, Y H:i" }}</td>
            </tr>
            {% empty %}
            <tr>
                <td colspan="6" style="text-align: center; padding: 2rem;">No scan results found.</td>
            </tr>
            {% endfor %}
        </tbody>
    </table>
...
file_path = r'd:\gibi\pluuto\events\templates\events\admin\scan_result_list.html'
